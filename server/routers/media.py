...
def dynamic_list_pics(folder_path, page):
    # Check if the specified folder exists
    full_path = os.path.abspath(folder_path)
    print('Full Path:', full_path)
    if not os.path.exists(folder_path):
...
@media_bp.route('/pics/dynmaic_catalog/<int:page>', methods=['GET'])
def dynmaic_catalog_pics(page):
    print('catalog!')
    # return os.getcwd()
    return dynamic_list_pics('catalog-faces', page)
